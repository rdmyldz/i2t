...
func main() {
	...
	// err = handle.SetVariable("tessedit_char_whitelist", " 123456789ABCDEFGHIJKLMNOPQRSTUVWXYZİĞÜŞÇÖabcdefghijklmnopqrstuvwxyzığüşçö")
	// if err != nil {
	// 	log.Fatalln(err)
	// }
	...
	*/
	for _, pic := range pictures {
		...
		texts, err := handle.ProcessImage(pic)
		if err != nil {
			log.Printf("error processing %s: %v", pic, err)
			continue
		}
